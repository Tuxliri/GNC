...
station(2).minimumElevation = deg2rad(20);  % minimum elevation of the
                                            % tracked object [rad]
station(2).R = diag([0.0005 0.0005]);  % Noise meaurement matrix for
...
station(3).minimumElevation = deg2rad(10);  % minimum elevation of the
                                            % tracked object [rad]
station(3).R = diag([0.001 0.001]);  % Noise meaurement matrix
...
for i = 1:n
    ...
    [azimuth, elevation, range, ~] = antenna_pointing(station(3).name,...
                                            tLastVisibility, xt);
    satVersor = [cos(azimuth)*cos(elevation) -sin(azimuth)*cos(elevation) sin(elevation)];
    
    % Compute the angle B/W the satellite and the antenna center
    angle(i) = acos(dot(antennaCenterVersor,satVersor));
...
insideFoVPoints = sum(rad2deg(angle)<(station(3).FoV));
...
tf = cspice_str2et('2021-Nov-23 14:00:00.0000 TDB');
objective = @(x) costFcn(x,station,t0,tf,mu);
x0 = x0mean;
options = optimoptions('lsqnonlin','Algotithm',...
...
for window = 1:rows
    ...
    measurements(window).measurementTimes = tspan;
end
...
% IMPROVEMENT: implement the WEIGHTED LEAST SQUARE USING SLIDE 22 as a
% reference
tspan = t0:60:tf;
...
for k=1:length(station)
    T = struct2table(station(k).measurements);
    ...
    res = residual(xx,station,measurementTimes,measurements,tspan);
    
    costVect = [costVect; res];

end

end

function res = residual(xx,station,measurementTimes,measurements,tspan)
...
%     costVect = zeros(3*length(usedMeasurementsIndexes),1);
    ...
    for i=1:length(usedMeasurementsIndexes)
        ...
        res = estimatedMeasurement'-usedMeasurements(i,:)';
